...
//error_reporting(0);
		...
		$getSQLs = "select * from client_isbpos.production where upload = 1 and shop_id = '".$_SESSION["s_id"]."' and pro_status = 2";
		echo $getSQLs."<br>";
		mysql_select_db($database_dbconfig, $dbconfig);
		$Resultgs = mysql_query($getSQLs, $dbconfig) or die(mysql_error());	 
		while($rowgs = mysql_fetch_assoc($Resultgs))
		{
			...
			$getSQL = "select * from isbpos.production where client_pro_id = '".$cauto_pro_id."'";
			echo $getSQL."<br>";
			mysql_select_db($database_dbconfig, $dbconfig);
			$Resultg = mysql_query($getSQL, $dbconfig) or die(mysql_error());	 
			$rowg = mysql_fetch_assoc($Resultg);
			$sauto_pro_id   = $rowg["pro_id"];
			$supload  		= $rowg["upload"];
			if($rowg>0)
			{
				if($supload==1)
				{
					//echo "Added";
					 $updateSQLc = "UPDATE isbpos.production set upload = 2 where client_pro_id = '$cauto_pro_id'";
					//echo $updateSQLc."<br>";
					mysql_select_db($database_dbconfig, $dbconfig);
					$Resultc = mysql_query($updateSQLc, $dbconfig) or die(mysql_error());
				}
			...
			{
		  $insertSQLs = "INSERT INTO isbpos.production
			(pro_date,daily_rate,pr_qty,pr_weight,shop_id,dress_weight,pro_datetime,perc,upload,client_pro_id,pro_status) 
			VALUES('$pro_date','$daily_rate','$pr_qty','$pr_weight','$shop_id','$dress_weight','$pro_datetime','$perc','2','$cauto_pro_id','0')";
				echo $insertSQLs."<br>";
				mysql_select_db($database_dbconfig, $dbconfig);
				$Result1 = mysql_query($insertSQLs, $dbconfig) or die(mysql_error());
				...
				if($Result1)
				{
					$getSQLcn = "select * from isbpos.production where client_pro_id = '".$cauto_pro_id."' and upload = 2";
					echo $getSQLcn."<br>";
					mysql_select_db($database_dbconfig, $dbconfig);
					$Resultgcn = mysql_query($getSQLcn, $dbconfig) or die(mysql_error());	 
					$rowgcn = mysql_fetch_assoc($Resultgcn);
					$snauto_pro_id  = $rowgcn["pro_id"];

					$updateSQL = "UPDATE client_isbpos.production set upload = 2, server_pro_id = '$snauto_pro_id'
					where pro_id = '$cauto_pro_id' ";
					echo $updateSQL."<br>";
					mysql_select_db($database_dbconfig, $dbconfig);
					$Resultu = mysql_query($updateSQL, $dbconfig) or die(mysql_error());
					...
					///////////////////////////////batches/////////////////////////////////////
					 ...
					 $getSQLsb = "select * from client_isbpos.production_batches where upload = 1 and pro_id = '".$cauto_pro_id."' and pb_status = 2";
					echo $getSQLsb."<br>";
					mysql_select_db($database_dbconfig, $dbconfig);
					$Resultgsb = mysql_query($getSQLsb, $dbconfig) or die(mysql_error());	 
					while($rowgsb = mysql_fetch_assoc($Resultgsb))
					{
						...
						$getSQLb = "select * from isbpos.production_batches where client_pb_id = '".$cauto_pb_id."'";
						echo $getSQLb."<br>";
						mysql_select_db($database_dbconfig, $dbconfig);
						$Resultgb = mysql_query($getSQLb, $dbconfig) or die(mysql_error());	 
						$rowgb = mysql_fetch_assoc($Resultgb);
						$sauto_pb_id    = $rowgb["pb_id"];
						$suploadb  		= $rowgb["upload"];
						if($rowgb>0)
						{
							if($suploadb==1)
							{
								//echo "Added";
								$updateSQLc = "UPDATE isbpos.production_batches set upload = 2 where client_pb_id = '$cauto_pb_id'";
								mysql_select_db($database_dbconfig, $dbconfig);
								$Resultc = mysql_query($updateSQLc, $dbconfig) or die(mysql_error());
							}
							else
						...
						else
						{
							  ...
							  $insertSQLs = "INSERT INTO isbpos.production_batches
						(pb_qty,pb_weight,pro_id,pb_datetime,upload,client_pb_id) 
			VALUES('$pb_qty','$pb_weight','$snauto_pro_id','$pb_datetime','2','$cauto_pb_id')";
							echo $insertSQLs."<br>";
							mysql_select_db($database_dbconfig, $dbconfig);
							$Resultba = mysql_query($insertSQLs, $dbconfig) or die(mysql_error());
						}
							...
							if($Resultba)
							{
								$getSQLcn = "select * from isbpos.production_batches where client_pb_id = '".$cauto_pb_id."' and upload = 2";
								echo $getSQLcn."<br>";
								mysql_select_db($database_dbconfig, $dbconfig);
								$Resultgcn = mysql_query($getSQLcn, $dbconfig) or die(mysql_error());	 
								$rowgcn = mysql_fetch_assoc($Resultgcn);
								$snnauto_pb_id  = $rowgcn["pb_id"];

								echo $updateSQL = "UPDATE client_isbpos.production_batches set upload = 2, server_pb_id = '$snnauto_pb_id'
								where pb_id = '$cauto_pb_id' ";
								mysql_select_db($database_dbconfig, $dbconfig);
								$Resultu = mysql_query($updateSQL, $dbconfig) or die(mysql_error());
							}
					...
					///////////////////////////////products/////////////////////////////////////
					 ...
					 $getSQLsb = "select * from isbpos.production_prod where upload = 1 and pro_id = '".$cauto_pro_id."' and pp_status = 2";
					echo $getSQLsb."<br>";
					mysql_select_db($database_dbconfig, $dbconfig);
					$Resultgsb = mysql_query($getSQLsb, $dbconfig) or die(mysql_error());	 
					while($rowgsb = mysql_fetch_assoc($Resultgsb))
					{
						...
						$getSQLp = "select * from isbpos.production_prod where client_pp_id = '".$cauto_pp_id."'";
						echo $getSQLp."<br>";
						mysql_select_db($database_dbconfig, $dbconfig);
						$Resultgp = mysql_query($getSQLp, $dbconfig) or die(mysql_error());	 
						$rowgp = mysql_fetch_assoc($Resultgp);
						$sauto_pp_id    = $rowgp["pp_id"];
						$suploadp  		= $rowgp["upload"];
						if($rowgp>0)
						{
							if($suploadp==1)
							{
								//echo "Added";
								$updateSQLc = "UPDATE isbpos.production_prod set upload = 2 where client_pp_id = '$cauto_pp_id'";
								mysql_select_db($database_dbconfig, $dbconfig);
								$Resultc = mysql_query($updateSQLc, $dbconfig) or die(mysql_error());
							}
							else
						...
						else
						{
							  ...
							  $insertSQLs = "INSERT INTO isbpos.production_prod
						(prod_id,p_qty,p_weight,pro_id,shop_id,upload,client_pp_id) 
			VALUES('$prod_id','$p_qty','$p_weight','$snauto_pro_id','$shop_id','2','$cauto_pp_id')";
							echo $insertSQLs."<br>";
							mysql_select_db($database_dbconfig, $dbconfig);
							$Resultba = mysql_query($insertSQLs, $dbconfig) or die(mysql_error());
						}
							...
							if($Resultba)
							{
								$getSQLcn = "select * from isbpos.production_prod where client_pp_id = '".$cauto_pp_id."' and upload = 2";
								echo $getSQLcn."<br>";
								mysql_select_db($database_dbconfig, $dbconfig);
								$Resultgcn = mysql_query($getSQLcn, $dbconfig) or die(mysql_error());	 
								$rowgcn = mysql_fetch_assoc($Resultgcn);
								$snnauto_pp_id  = $rowgcn["pp_id"];

								echo $updateSQL = "UPDATE client_isbpos.production_prod set upload = 2, server_pp_id = '$snnauto_pp_id'
								where pp_id = '$cauto_pp_id' ";
								mysql_select_db($database_dbconfig, $dbconfig);
								$Resultu = mysql_query($updateSQL, $dbconfig) or die(mysql_error());
							}
					...
					////////////////////////////////////////////////////////////////////////////////
					 ...
					 $insertSQL = "CALL isbpos.`pro_stock`($snauto_pro_id)";
					mysql_select_db($database_dbconfig, $dbconfig);
					$Result1 = mysql_query($insertSQL, $dbconfig) or die(mysql_error());
					
					$insertSQL = "CALL isbpos.`pro_stock_sub`($snauto_pro_id)";
					mysql_select_db($database_dbconfig, $dbconfig);
					$Result1 = mysql_query($insertSQL, $dbconfig) or die(mysql_error());
					
					$insertSQL = "CALL isbpos.`pro_update`($snauto_pro_id)";
					mysql_select_db($database_dbconfig, $dbconfig);
					$Result1 = mysql_query($insertSQL, $dbconfig) or die(mysql_error());

					$insertSQL = "CALL isbpos.`pb_update`($snauto_pro_id)";
					mysql_select_db($database_dbconfig, $dbconfig);
					$Result1 = mysql_query($insertSQL, $dbconfig) or die(mysql_error());

					$insertSQL = "CALL isbpos.`pp_update`($snauto_pro_id)";
					mysql_select_db($database_dbconfig, $dbconfig);
					$Result1 = mysql_query($insertSQL, $dbconfig) or die(mysql_error());
